...
router.get("/getQuizData/:id", (req, res) => {
  ...
  History.find({
    ...
    //   // $gte: new Date(`${today.toISOString("uk").substring(0, 10)}T00:00:00.000Z`),
    ...
    .then((result) => {
      ...
      result.forEach(element => {
        console.log(element.dateOfQuiz)
      });
      ...
      if (result.length > 0) {
        ...
        result.forEach((element) => {
          // console.log((element.dateOfQuiz).getDay(), ":", today.getDay())
          if (element.dateOfQuiz.toISOString().split("T")[0] === today) {

            temp = { item: element, totalScore: result.length }
            // res.json({ item: element, totalScore: result.length });
          }
          //  else {
          //   res.json(null);
          // }
      ...
      // console.log("result-> ",result.dateOfQuiz);
      // result == null ? res.json(result) : res.json(result);
...
router.get("/getDashboardData", (req, res) => {
  const data = req.body;
  History.findOne({ userId: data.userId })
    .sort({ dateOfQuiz: -1 })
    .limit(1)
    .then((result) => {
      res.json(result);
